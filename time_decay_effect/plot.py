...
hist_decay = np.load(os.path.join(datapath, 'hist_decay.npy'))
hist_nodecay = np.load(os.path.join(datapath, 'hist_nodecay.npy'))
...
ax.set_title(r"\textbf{With Decay}", fontsize=fontsize)
...
# ax.plot(hist[:, 0], hist[:, 1], color='tab:blue', alpha=0.5, lw=2)
line = colored_line(hist[:, 0], hist[:, 1], np.arange(nbDataPoints), ax, cmap='plasma', linewidth=2)
...
plt.savefig(os.path.join(datapath, 'decay.pdf'), bbox_inches='tight', pad_inches=0, dpi=600)
plt.show()
